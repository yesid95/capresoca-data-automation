...
de la Base de Datos Única de Afiliados (BDUA) en Capresoca EPS; y la clase DataCleaner, diseñada
...
de limpieza y validación de datos, asegurando que los reportes cumplan con los requicitos de ADRES
    ...
    **Uso:**
    ...
    def prioritize_population_markers(self, col_name: str) -> pd.DataFrame:
        ...
        def _process_value(value: str) -> str:
            ...
            # Regla 2: Eliminar sufijos de SISBEN III y IV que acompañan a un listado censal
            processed_value = re.sub(r"-SIV\([^\)]*\)|-SIII\([^\)]*\)", "", original_value)

            # Regla 3: Si después de limpiar solo queda una marca de SISBEN III, se elimina
            if re.fullmatch(r"SIII\([^\)]*\)", processed_value.strip()):
                return ""

            # Regla 4: Eliminar el prefijo de listado censal 'LC(0)-' que indica sin clasificación
            ...
            # Regla 6: Normalizar y priorizar listados censales múltiples (p. ej., "LC(9|17)")
            def _prioritize_lc(match: re.Match) -> str:
                ...
                # Iterar sobre la jerarquía definida en la clase para encontrar la mayor prioridad
...
class DataCleaner:
    ...
    def process_date_column(self, column_name: str) -> pd.DataFrame:
        ...
        if is_datetime(column_series):
            print("INFO: La columna ya está en formato datetime. No se requiere conversión.")
            return df_copy
        ...
        if errors_found > 0:
            print(f"ADVERTENCIA: Se encontraron {errors_found} fechas no válidas que fueron convertidas a NaT.")
